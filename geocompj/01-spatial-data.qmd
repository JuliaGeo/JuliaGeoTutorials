...
We will focus on the GeoInterface.jl ecosystem for working with vector data and the Rasters.jl package for working with raster data, which is also integrated with GeoInterface.
...
It also provides an interface to three mature libraries for geocomputation which, in combination, represent a strong foundation on which many geographic applications are built (including QGIS and R's spatial ecosystem):
...
For example, the two following commands return exactly the same result, a `GeoSeries` with country bounding box polygons (using the `.envelope` method).
...
A point is simply a coordinate in 2D, 3D, or 4D space such as shown in @fig-point.
...
The raster header defines the coordinate reference system, the the origin and the resolution. 
The origin (or starting point) is typically the coordinate of the lower-left corner of the matrix. 
The metadata defines the the origin, and the cell size, i.e., resolution.
...
<!-- md: "Map Projections" (1993) - there is no citation and I'm not familiar with this source -->
